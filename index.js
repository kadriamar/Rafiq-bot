...
function handleMessage(senderId, text) {
  ...
  } else if (msg.includes('طلب') || msg.includes('اشتري')) {
    startOrder(senderId);
...
function startOrder(senderId) {
  ...
  sendMessage(senderId, { text: 'طلب تقسيط جديد\n\nما هو اسمك الكامل؟' });
}
...
function handleOrderSession(senderId, text) {
  ...
  switch (session.step) {
    ...
    case 'branch':
      session.branch = text;
      session.step = 'photo';
      sendMessage(senderId, { text: 'أرسل صورة المنتج أو اكتب "تخطي"' });
      break;
    case 'photo':
      session.photo = null;
      session.step = 'done';
...
function sendWelcome(senderId) {
  sendMessage(senderId, {
    ...
    quick_replies: [
      { content_type: 'text', title: 'طلب تقسيط', payload: 'START_ORDER' },
